...
int CIAManager::GetClosestNode(const Vect3f& _vPosition, int _iMaxDistance)
{
  //TODO optimitzacions
  return m_pGraph->GetClosestNode(_vPosition, _iMaxDistance);
}
...
vector<CGraphNode*> CIAManager::GetClosestCobertura   (const Vect3f& _vPosition, int _iMaxDistance)
{
  vector<CGraphNode*> Path_;

  int l_iOrigin      = GetClosestNode(_vPosition, _iMaxDistance);
  
  if(l_iOrigin == INVALID_GRAPH_NODE_INDEX)
    return Path_;
