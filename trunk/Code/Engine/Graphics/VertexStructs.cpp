  ...
  if(s_VertexDeclaration==NULL)
  {
    D3DVERTEXELEMENT9 l_VertexDeclaration[] =
    {
      ...
      */
      ...
      { 1, sizeof(float)*8,  D3DDECLTYPE_FLOAT2,   D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD, 2 },
      { 1, sizeof(float)*10, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_COLOR,  0 },
      D3DDECL_END()
    };
    HRESULT result = RENDER_MANAGER->GetDevice()->CreateVertexDeclaration(l_VertexDeclaration, &s_VertexDeclaration);
    assert(result == D3D_OK);
