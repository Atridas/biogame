    ...
    if(l_treeParticleEmitters.Exists())
    {
      ...
      for(int i = 0; i < l_iNumChildrenEmitters; i++)
      {
        ...
        Vect3f l_vVec3;
       	...
       	l_pCore->m_szId = l_treeParticleEmitter.GetPszISOProperty("id" ,"",true);
			  l_pCore->m_fMinEmitRate = l_treeParticleEmitter.GetFloatProperty("MinEmitRate",1,true);
			  l_pCore->m_fMaxEmitRate = l_treeParticleEmitter.GetFloatProperty("MaxEmitRate",l_pCore->m_fMinEmitRate,false);
			  ...
			  //l_pCore->m_pTexParticle = CORE->GetTextureManager()->GetResource(l_treeParticleEmitter.GetPszProperty("TexParticle","",true));
        l_pCore->m_fLife1 = l_treeParticleEmitter.GetFloatProperty("Life1", 1, true);
        l_pCore->m_fLife2 = l_treeParticleEmitter.GetFloatProperty("Life2", l_pCore->m_fLife2, false);
        l_pCore->m_fAngle1 = l_treeParticleEmitter.GetFloatProperty("Angle1", 0, false);
        l_pCore->m_fAngle2 = l_treeParticleEmitter.GetFloatProperty("Angle2", l_pCore->m_fAngle1, false);
        l_pCore->m_vGravity = l_treeParticleEmitter.GetVect3fProperty("Gravity",Vect3f(0.0f),false);
        l_pCore->m_vVel = l_treeParticleEmitter.GetVect3fProperty("Velocitate",Vect3f(1,1,1),false);
        l_pCore->m_bAnimated = false; // si te animacio despres la canvio
        l_pCore->m_bBucleInfinit=l_treeParticleEmitter.GetBoolProperty("Bucle",true,false);
        if(l_pCore->m_bBucleInfinit==false)
        {
          l_pCore->m_iNumBucle=l_treeParticleEmitter.GetIntProperty("NumBucle",1,true);
          l_pCore->m_fRebootEmitter1=l_treeParticleEmitter.GetFloatProperty("RebootEmitter1",1,true);
          l_pCore->m_fRebootEmitter2=l_treeParticleEmitter.GetFloatProperty("RebootEmitter2",l_pCore->m_fRebootEmitter1,false);
          l_pCore->m_bTotDeCop=l_treeParticleEmitter.GetBoolProperty("TotDeCop",true,true);
        }else
        {
          l_pCore->m_iNumBucle=0;
          l_pCore->m_fRebootEmitter1=0;
          l_pCore->m_fRebootEmitter2=0;
          l_pCore->m_bTotDeCop=false;
        }
        
			  

        l_pCore->m_vTimeColor.push_back(0);
        l_pCore->m_vColor.push_back(l_Event.m_Color1);
        l_pCore->m_vColor.push_back(l_Event.m_Color2);
        l_pCore->m_vTimeSize.push_back(0);
        l_pCore->m_vSize.push_back(l_Event.m_fMinSize);
        l_pCore->m_vSize.push_back(l_Event.m_fMaxSize);
        l_pCore->m_vTextureAnimation.push_back(l_pCore->m_pTexParticle);
        ...
        int num = l_treeParticleEmitter.GetNumChildren();
            ...
            if(l_treeParticleEmittersSizes.Exists())
            {
              ...
              for(int j = 0; j < l_iNumChildrenSizes; j++)
              {
                ...
                l_pCore->m_vTimeSize.push_back(l_Event.m_fTime);
                l_pCore->m_vTimeSizeInterpolation.push_back(l_Event.m_fTimeInterpolation);
                l_pCore->m_vSize.push_back(l_Event.m_fMinSize);
                l_pCore->m_vSize.push_back(l_Event.m_fMaxSize);
            ...
            if(l_treeParticleEmittersColors.Exists())
            {
              ...
              for(int j = 0; j < l_iNumChildrenColors; j++)
              {
                ...
                l_pCore->m_vTimeColor.push_back(l_Event.m_fTime);
                l_pCore->m_vTimeColorInterpolation.push_back(l_Event.m_fTimeInterpolation);
                l_pCore->m_vColor.push_back(l_Event.m_Color1);
                l_pCore->m_vColor.push_back(l_Event.m_Color2);
            ...
            if(l_treeParticleEmittersAnimateds.Exists())
            {
              ...
              for(int j = 0; j < l_iNumChildrenAnimateds; j++)
              {
                ...
                l_pCore->m_vTimeAnimatedInterpolation.push_back(l_Event.m_fTimeInterpolation);
                //l_pCore->m_pTexParticle = CORE->GetTextureManager()->GetResource(l_treeParticleEmittersAnimated.GetPszProperty("TexParticle","",true));
			          ...
			          l_pCore->m_vFilesColumnes.push_back(l_Event.m_iTexNumFiles);
			          l_pCore->m_vFilesColumnes.push_back(l_Event.m_iTexNumColumnes);
                l_pCore->m_vTextureAnimation.push_back(l_pCore->m_pTexParticle);
        ...
        //AddResource(l_pCore->m_szId,l_pCore);
      }
    }
  }
  return true;*/
...
bool CParticleCore::Reload(const string& _szFileName)
{
  Release();
  //return LoadParticleCore(_szFileName);
  return true;
}
