    ...
    if(l_treeParticleEmitters.Exists())
    {
      ...
      for(int i = 0; i < l_iNumChildren; i++)
      {
        ...
        if(l_treeParticleEmittersColors.Exists())
        {
          ...
          CXMLTreeNode l_treeParticleEmittersDiredtions = l_XMLParticles["Directions"];
	...
	if(strcmp(l_XMLParticles.GetName(),"Particles") == 0)
  {
    ...
    if(l_treeParticleEmitters.Exists())
    {
      ...
      for(int i = 0; i < l_iNumChildren; i++)
      {
        ...
        CXMLTreeNode l_treeParticleEmittersProperties = l_XMLParticles["Properties"];

        if(l_treeParticleEmittersProperties.Exists())
        { 
          LOGGER->AddNewLog(ELL_INFORMATION,"CParticleManager::Load Loading %d Propietats de les fonts emisores.", l_iNumChildren);

          if(l_treeParticleEmittersProperties.IsComment())
				    continue;

			    
        
          l_pInfo->m_szId = l_treeParticleEmittersProperties.GetPszISOProperty("id" ,"");
			    l_pInfo->m_fMinEmitRate = l_treeParticleEmittersProperties.GetFloatProperty("MinEmitRate");
			    l_pInfo->m_fMaxEmitRate = l_treeParticleEmittersProperties.GetFloatProperty("MaxEmitRate");
			    l_vVec4 = l_treeParticleEmittersProperties.GetVect4fProperty("Color1",Vect4f(0.0f),true);
			    l_Event.m_Color1 = D3DXCOLOR(l_vVec4.x,l_vVec4.y,l_vVec4.z,l_vVec4.w);
			    l_vVec4 = l_treeParticleEmittersProperties.GetVect4fProperty("Color2",Vect4f(0.0f),true);
			    l_Event.m_Color2 = D3DXCOLOR(l_vVec4.x,l_vVec4.y,l_vVec4.z,l_vVec4.w);
			    l_pInfo->m_fMinSize = l_treeParticleEmittersProperties.GetFloatProperty("MinSize");
			    l_pInfo->m_fMaxSize = l_treeParticleEmittersProperties.GetFloatProperty("MaxSize");
			    l_vVec3 = l_treeParticleEmittersProperties.GetVect3fProperty("Direction1",Vect3f(3.0f));
          l_Event.m_vSpawnDir1 = D3DXVECTOR3(l_vVec3.x,l_vVec3.y,l_vVec3.z);
			    l_vVec3 = l_treeParticleEmittersProperties.GetVect3fProperty("Direction2",Vect3f(0.0f));
          l_Event.m_vSpawnDir2 = D3DXVECTOR3(l_vVec3.x,l_vVec3.y,l_vVec3.z);
          l_pInfo->m_pTexParticle = CORE->GetTextureManager()->GetResource(l_treeParticleEmittersProperties.GetPszProperty("TexParticle",""));
          l_pInfo->m_fLife1 = l_treeParticleEmittersProperties.GetFloatProperty("Life1");
          l_pInfo->m_fLife2 = l_treeParticleEmittersProperties.GetFloatProperty("Life2");
